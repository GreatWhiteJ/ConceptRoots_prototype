...
export default function YoutubePlaylist(data) {
  ...
  const tryFetchTranscript = videoid => {
    return new Promise((resolve, reject) => {
      ...
      Axios.get("http://video.google.com/timedtext?lang=en&v=" + videoid).then(
        response => {
          if (!response.data) {
      ...
      if (
        ...
        if (current_time && tcopy) {
          let current_script = tcopy.filter(script => {
            ...
            return (
              startTime < current_time && current_time < startTime + duration
            );
          });
      ...
      <div className="ytp_container">
        <div className="ytp_header">
          ...
          {typeof channel != "undefined" &&
            ...
            typeof channel.items[0] != "undefined" && (
              <div>
        ...
        <div className="ytp_meta_container">
          ...
          <div
            className="ytp_meta_data"
            style={multiitems ? { width: "100%" } : null}
          >
            {multiitems && (
              <div>
                <p>Autoplay</p>
                  ...
                  {transcript.map(script => {
                    return (
                      <span
                        ...
                        className={script.active ? "ytp_active" : null}
                        data-start={script.$.start}
                        data-duration={script.$.dur}
                      >
                        {" " + script._ + " "}
                      </span>
                    );
